...
export function PorteriaSalida() {
  ...
  return (
    <div className={styles.page}>
      ...
      <div style={{ display: 'flex', borderBottom: '2px solid var(--border)', marginBottom: 16 }}>
        <button
          ...
          style={{
            ...
            color: tab === 'salida' ? 'var(--bp800)' : 'var(--text-lighter)',
            borderBottom: tab === 'salida' ? '2px solid var(--bp800)' : '2px solid transparent',
            marginBottom: -2,
          }}
        >
          🚗 Salida
        ...
        <button
          ...
          style={{
            flex: 1, padding: '10px 0', border: 'none', background: 'none',
            fontWeight: 600, fontSize: 14, cursor: 'pointer',
            color: tab === 'retorno' ? 'var(--bp800)' : 'var(--text-lighter)',
            borderBottom: tab === 'retorno' ? '2px solid var(--bp800)' : '2px solid transparent',
            marginBottom: -2,
          }}
        >
          🏁 Retorno
      ...
      {tab === 'salida'  && <TabSalida  usuario={usuario} />}
      {tab === 'retorno' && <TabRetorno usuario={usuario} />}
    </div>
  )
}
...
function TabSalida({ usuario }) {
  ...
  async function buscar(codigoParam) {
    ...
    try {
      ...
      setLoading(false)
      setError('Error al buscar: ' + (e?.message ?? 'Intentá de nuevo.'))
    }
  }
  ...
  return (
    <div className={styles.content}>
      <div style={{ display: ticket || confirmado ? 'none' : 'block' }}>
        {mostrarScanner ? (
          <QrScanner
            onResult={(texto) => { buscar(texto); setMostrarScanner(false) }}
            onError={(err) => { setMostrarScanner(false); setError('Error camara: ' + (err?.message ?? err)) }}
          />
        ) : (
          <div className={styles.scanArea} onClick={() => setMostrarScanner(true)} style={{ cursor: 'pointer' }}>
        ...
        <Field label="Código de ticket">
          <div className={styles.searchRow}>
            <Input value={codigo} onChange={e => setCodigo(e.target.value)} placeholder="Ej: VEH-2026-0001" onKeyDown={e => e.key === 'Enter' && buscar()} />
        ...
        {resultados.length > 1 && (
          <>
            ...
            {resultados.map(t => (
              <div key={t.id} onClick={() => { setTicket(t); setResultados([]) }} className={styles.resultCard}>
      ...
      {ticket && !confirmado && (
        <>
          <EstadoBannerSalida estado={ticket.estado} />
          <SummaryCard>
            ...
            {ticket.jefe_comentario && <SummaryRow label="Nota del jefe" value={ticket.jefe_comentario} />}
          </SummaryCard>
          {error && <Banner type="error" icon="⚠️">{error}</Banner>}
  ...
  async function confirmar() {
    ...
    try {
      ...
      await supabase.functions.invoke('notificar-retorno', {
        body: { colaboradorId: ticket.colaborador_id, ticketId: ticket.id },
      }).catch(() => {})
      setConfirmado(true)
    } catch (e) {
      setError('Error al confirmar: ' + (e?.message ?? 'Intentá de nuevo.'))
    ...
    <div className={styles.content}>
      ...
      {confirmado && (
        <>
          ...
          <Banner type="info" icon="ℹ️">El colaborador debe completar los datos de retorno en su app.</Banner>
          <Btn variant="primary" onClick={reset} full>← Nuevo escaneo</Btn>
        </>
      )}
    </div>
...
function EstadoBannerSalida({ estado }) {
